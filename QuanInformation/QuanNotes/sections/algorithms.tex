...
\begin{gather*}
  F\ket{n} = \frac{1}{\sqrt{2^n}} \sum_{y=0}^{2^n-1} \zeta^{xy} \ket{y} = \frac{1}{\sqrt{2^n}} \sum_{y_0=0}^{1}\sum_{y_2=0}^{1}...\sum_{y_{n-1}=0}^{1} \zeta^{x(\sum_{j=1}^n y_j2^j)} \ket{y_{n-1}} ...\ket{y_0} = \\
  ...
  \bigotimes_{i=0}^{n-1} (\ket{0} + e^{2\pi i(x_{n- 1- k}2^{-1} +x_{n-2}2^{i-2} + \ldots + x_{0} 2^{i-n})})\ket{1})
\end{gather*}
%
The last equality follows from the fact that any bit shift past $n-1-k$ times will result in an even power of $e^{2\pi i}$ which will always be equal one.
%
From this expression, we can construct the quantum circuit implementing the Quantum Fourier Transform:
